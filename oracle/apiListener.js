...
export default function start(watchedStakes, contract, interval,signTransaction) {
  setInterval(() => { checkGamesChanged(watchedStakes, contract,signTransaction) }, interval)
}

function checkGamesChanged(watchedStakes, contract,signTransaction) {
  ...
  if (watchedSingleGames.length > 0) {
    fetch(`http://127.0.0.1:8000/games?id=${watchedSingleGames.map(game => game.apiId).join('&id=')}`)
      ...
      .then(data => { compareAndUpdate(data, watchedStakes, contract,signTransaction) })
  }
...
function compareAndUpdate(data, watchedStakes, contract, signTransaction) {
      ...
      for (const matchingStake of matchingStakes) {
        contract.methods.getPlayer(matchingStake.horse).call()
          .then(player => {
            ...
            signTransaction(matchingStake.id, amountWon)
          })
